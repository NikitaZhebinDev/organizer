...
public class MyIntentService extends IntentService {
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    if (intent.getStringExtra("Reboot").compareTo("Yes") == 0) {
      /**   RESTORE  */
      /**   Now we have to READ ALL of our Tasks  */
      ...
      String title = getString(R.string.task_at) + " " + DateUtils.formatDateTime(this,
          System.currentTimeMillis(), DateUtils.FORMAT_SHOW_TIME);

      Intent notificationIntent = new Intent(this, MainMenu.class);
      notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);

      PendingIntent contentIntent = PendingIntent.getActivity(this, id + 850,
      ...
      if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
        ...
        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder
            ...
            .setSmallIcon(R.drawable.ic_notification_icon) // app icon
        ...
        if (alarms.compareTo("true") == 0 || alarms.compareTo("false") == 0 || alarms.compareTo("") == 0) {
  ...
  private void sendNotification(java.util.Calendar calendar, String id, String repeat, String message, Context context) {
    ...
    // TODO: ONE REAQUEST CODE FOR ALL.... That's bad !!!!!!!!!!!!!!!!!!
    PendingIntent pendingIntent = PendingIntent.getBroadcast(context, (newID + 87658),
    ...
    if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[0]) == 0) {
      // 0 mins
      timeBefore = 0;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[1]) == 0) {
      // 5 mins
      timeBefore = 60000 * 5;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[2]) == 0) {
      // 15 mins
      timeBefore = 60000 * 15;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[3]) == 0) {
      // 30 mins
      timeBefore = 60000 * 30;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[4]) == 0) {
      // 1 hour
      timeBefore = 60000 * 60;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[5]) == 0) {
      // 2 hours
      timeBefore = 60000 * 60 * 2;
    } else if (myPreference.getString("taskNotification", "0").compareTo(
        context.getResources().getStringArray(R.array.task_notification)[6]) == 0) {
      // 3 hours
      timeBefore = 60000 * 60 * 3;
    }

    try {
      if (repeat.compareTo(DataBaseLists.listRepeatNames[0]) == 0) {
        // Don't repeat
        alarmManager.set(RTC_WAKEUP, calendar.getTimeInMillis() - timeBefore, pendingIntent);
      } else if (repeat.compareTo(DataBaseLists.listRepeatNames[1]) == 0) {
        // Each hour
        alarmManager.setRepeating(RTC_WAKEUP, calendar.getTimeInMillis() - timeBefore, INTERVAL_HOUR, pendingIntent);
      } else if (repeat.compareTo(DataBaseLists.listRepeatNames[2]) == 0) {
        // Daily
        alarmManager.setRepeating(RTC_WAKEUP, calendar.getTimeInMillis() - timeBefore, INTERVAL_DAY, pendingIntent);
      } else if (repeat.compareTo(DataBaseLists.listRepeatNames[3]) == 0) {
        // Weekly
        alarmManager.setRepeating(RTC_WAKEUP, calendar.getTimeInMillis() - timeBefore, INTERVAL_DAY * 7, pendingIntent);
      } else if (repeat.compareTo(DataBaseLists.listRepeatNames[4]) == 0) {
        // Yearly
        alarmManager.setRepeating(RTC_WAKEUP, calendar.getTimeInMillis() - timeBefore, INTERVAL_DAY * 365, pendingIntent);
      }
    } catch (Exception e) {
      // TODO
    }
